...
| `p2_pipeline_current_compact.svg` | 当前可编辑流程图 | 用于 PPT 排版的紧凑 SVG；属于实验复盘后的框架设计，不代表已经用该结构重新训练。 |
...
| `p2_eval_comparison.png` | 自适应策略与固定小跑对比 | 同检查点、同环境、同随机种子的严格成对评测；并排展示上坡 1.0/1.5 m/s 两套策略的速度误差、接触滑移、冲击和机械功率实测均值。 |
| `p2_ood_generalization.png` | 粗糙地面与踏石测试 | 展示五项原始物理指标相对固定小跑的改善百分比；按五项等权平均排序仅用于汇总展示，不作为训练奖励或显著性结论。 |
...
历史流程图草稿统一放在 `drafts/`。其中旧图包含后来放弃的物理量解码器、重复信息输入
或过密版式，只用于追溯设计过程，不应再放入 PPT。具体边界见 `drafts/README.md`。
当前 SVG 应直接插入 PowerPoint，或由矢量编辑器导出位图；仓库不再保留容易与旧结构混淆的
重复 PNG 预览。
