...
def add_step_stats(stats, env, action, reward, done, info):
    ...
    stats["action_clip_sum"] += float((torch.abs(residual) > 0.98).float().mean(dim=1).sum().item())
...
def main():
    ...
    all_specs = read_task_specs(args.task_map, style_reward_scale=0.0)
    ...
    print(
        ...
        f"selector_only={selector_only}, eval_items={len(eval_items)}"
    )
